...
class MNISTDataloader():
    ...
    def process_torch_geometric(self, data:torch.tensor, num_samples:int, k:int):
        """
        ...
        A, edge_weights = self.get_dist_adjacency(x, y, k, H, W, symmetric=True)
    ...
    def get_dist_adjacency(self, x:torch.tensor, y:torch.tensor, k:int, H:int, W:int, symmetric:bool):
        """
        ...
        delX = A*x_dist/H # this doesn't have self loops; diagonal elements are zero; [num_samples,num_samples]
        delY = A*y_dist/W # this doesn't have self loops; diagonal elements are zero; [num_samples,num_samples]
        # NOTE (R, theta); OR learn edge weight matrix
