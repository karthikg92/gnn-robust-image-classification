...
class Trainer():
    ...
    def __init__(self, data, params):
        ...
        self.train_step, self.accuracy, self.summary= e2e_v1(self.images , self.labels, self.learning_rate , \
         self.training_flag , params)
        ...
        # Validation
        self.images_val, self.labels_val = self.data.validation()
        self.best_val=0
    ...
    def train(self):
        ...
        for i in range(N_iter):
            img_batch, label_batch= self.data.next_train()
            current_learn_rate=self.lr_function(i, N_iter)
            input_dict = {self.images: img_batch, self.labels: label_batch, self.training_flag: True, self.learning_rate: current_learn_rate}
    ...
    def _test(self):
        ...
        N_batch = self.img_val.shape[0] // te_batch_size
        acc_list = []
        for i in range(N_batch):
            start = i * te_batch_size
            end = start + te_batch_size
            val_input_dict = {self.images: self.images_val[start:end, :, :], self.labels: self.labels_val[start:end,:], self.training_flag: False, self.learning_rate: 1e-3}
