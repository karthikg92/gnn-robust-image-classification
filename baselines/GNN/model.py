...
class SpatialGraphConv(MessagePassing):
    """
        Implementation of Spatial Graph Convolutional Network (SGCN)
        as defined in Processing of incomplete images by (graph) convolutional neural networks
        by Tomasz Danel et al. (ICML Artemiss Workshop 2020) https://openreview.net/pdf?id=wxYPUnMSQCR
    """
    def __init__(self, node_dim: int, edge_dim: int, num_filters: int, aggr: str = 'add', batch_norm: bool = False, bias: bool = True):
        super(SpatialGraphConv, self).__init__(aggr=aggr)
        self.edge_dim = edge_dim
        self.node_dim = node_dim
        self.batch_norm = batch_norm

        self.filters = nn.ModuleList()
        # adding batch norm just in case it is required
        if self.batch_norms:
            self.batch_norms = nn.ModuleList()
        for i in range(num_filters):
            self.filters.append(Linear(edge_dim, node_dim))
            self.batch_norms.append(BatchNorm1d(node_dim))
        # self.lin_u = Linear(edge_dim, node_dim)
        # self.bn = BatchNorm1d(node_dim)
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,
        ...
        out = self.propagate(edge_index, x, edge_attr=edge_attr, size=size)
        out = self.bn(out) if self.batch_norm else out
        return out

    def message(self, x_j, edge_attr) -> Tensor:
        """
            Return RELU( U[j-i] + b ) ⊙ x_j
            NOTE: here (j-i) are the edge attributes
            The aggregation sum will be determined by 'aggr'
            variable in class __init__
        """
        return self.lin_u(edge_attr).relu() * x_j
