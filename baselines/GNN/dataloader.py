...
class removePatch(object):
    ...
    def replace_patch(self, img: torch.tensor, ps: int):
        ...
        if self.patch_fill_type == 'mean':
            ...
            img[:, cx - ps:cx + ps + 1, cy - ps:cy + ps + 1] = self.mean_coords[:, cx - ps:cx + ps + 1,
                                                               cy - ps:cy + ps + 1]
...
class MNISTDataloader():
    @store_args
    def __init__(self, batch_size: int, train_val_split_ratio: float, seed: int, patch_fill_type: str, testing: bool,
                 download: bool = False):
        """
        ...
        self.train_dataloader, self.val_dataloader = train_val_split(self.dataset, self.train_val_split_ratio,
                                                                     self.batch_size)
...
if __name__ == "__main__":
    ...
    for method in ['mean', 'mask']:
        print('_' * 50);
        print(f'Showing images with {method} imputation');
        print('_' * 50)
